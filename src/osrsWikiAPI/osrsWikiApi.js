...
export const wikiLatest = async () => {
    ...
    return axios.get(`${WIKI_BASE_URL}latest`)
    .then(response => {
...
export const wikiTimeseries = async (id, timePeriod) => {
    return axios.get(`${WIKI_BASE_URL}/timeseries?timestep=${timePeriod}&id=${id}`)
    .then(response => {
        return response.data["data"];
    })
    .catch(error => {
